...
(function() {
  ...
  function ionicRatings () {
    return {
      ...
      '<span ng-show="count > -1">({{count}})</span>'+
      '</div>',
      scope: {
      ...
      link: function(scope, element, attrs) {
        ...
        scope.rating = scope.ratingsObj.rating;//added by me
        scope.minRating = scope.ratingsObj.minRating; //added by me
        scope.readOnly = scope.ratingsObj.readOnly || false;
        
          if(scope.ratingsObj.count != undefined)
              scope.count = scope.ratingsObj.count;
          else
              scope.count = -1;
        ...
        scope.$watch('ratingsObj.rating', function(newValue, oldValue) {
                if (newValue)
                {
                    ...
                    scope.prevRating = newValue;
                }
                    
            }, true);
          
        scope.$watch('ratingsObj.count', function(newValue, oldValue) {
                if (newValue)
                {
